...
if clustering_algorithm == "K-means" or clustering_algorithm == "gaussian" or clustering_algorithm == "birch" or clustering_algorithm == "Affinity":
...
if clustering_algorithm == "K-means":
    model = KMeans(n_clusters=2, init='random', max_iter=100, random_state=44)
elif clustering_algorithm == "gaussian":
    model = GaussianMixture(n_components=2, random_state=2, covariance_type="spherical", verbose=1)
elif clustering_algorithm == "birch":
    model = Birch(n_clusters=2, threshold=0.2)
else:
